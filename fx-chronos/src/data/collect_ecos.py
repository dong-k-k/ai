...
def collect_series(
    ...
    for row in rows:
        ...
        api_unit = str(row.get("UNIT_NAME", ""))
        if expected_unit_name is not None and api_unit != expected_unit_name:
            invalid_unit_name += 1
        ...
        if raw_value in (None, ""):
            ...
            continue
        try:
            raw_rate = float(str(raw_value))
            normalized_rate = convert_value(str(raw_value), expected_item_name)
        ...
        is_jpy_per_100 = "100엔" in item_name
        records.append(
            {
                "date": iso_date,
                # value는 기존 모델 입력과의 호환성을 위해 정규화 값을 유지한다.
                "value": normalized_rate,
                "raw_rate": raw_rate,
                "raw_unit": "KRW per 100 JPY" if is_jpy_per_100 else api_unit,
                "normalized_rate": normalized_rate,
                "normalized_unit": "KRW per JPY" if is_jpy_per_100 else api_unit,
                "stat_code": stat_code,
                "item_code": item_code,
                "item_name": item_name,
                "unit_name": api_unit,
    ...
    raw_out_path = RAW_DIR / f"{file_prefix}_{start_date}_{end_date}_{collected_at}.json"
    out_path = ECOS_PROCESSED_DIR / f"{file_prefix}_{start_date}_{end_date}.csv"
    ...
    quality_summary = {
        ...
        "non_target_item_count": mixed_item_count,
        "invalid_unit_name_count": invalid_unit_name,
        "missing_calendar_dates": missing_calendar_dates,
        "value_definition": {
            "raw": records[0]["raw_unit"] if records else None,
            "normalized": records[0]["normalized_unit"] if records else None,
            "value_column": "normalized_rate",
        },
    }
    ...
    print(f"대상 외 항목 혼입 수: {mixed_item_count}")
    print(f"단위 오류 수: {invalid_unit_name}")
    print(f"저장 경로: {out_path}")
    return out_path


def collect_jpykrw(start_date: str, end_date: str) -> Path:
    """Collect ECOS JPY/KRW and preserve both per-100-JPY and per-JPY values."""
    return collect_series(
        stat_code="731Y001",
        item_code="0000002",
        output_name="jpy_krw_krw_per_jpy.csv",
        description="JPY/KRW (KRW per JPY; raw ECOS value is KRW per 100 JPY)",
        start_date=start_date,
        end_date=end_date,
        cycle="D",
...
def collect_kr_treasury_3y(start_date: str, end_date: str) -> Path:
    """Collect the ECOS daily Korean 3-year Treasury yield in annual percent."""
    return collect_series(
        stat_code="817Y002",
        item_code="010200000",
        output_name="kr_treasury_3y_percent.csv",
        description="Korean 3-year Treasury yield (annual percent)",
        start_date=start_date,
        end_date=end_date,
        cycle="D",
        expected_item_name="국고채(3년)",
        file_prefix="kr_treasury_3y",
        expected_unit_name="연%",
    )
