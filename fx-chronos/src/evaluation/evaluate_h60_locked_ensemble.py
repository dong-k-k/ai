...
CONFIG_PATH = PROJECT_DIR / "configs" / "h60_ensemble_validation.json"
OUTPUT_STEM = "usd_krw_shrunk_ensemble_h60_ctx756_alpha0.5_validation_2018_2021"
...
def load_settings(config_path: Path = CONFIG_PATH) -> tuple[dict[str, object], Path]:
    ...
    if int(validation["prediction_length"]) != 60:
        raise RuntimeError("H60 고정 평가의 prediction length가 60이 아닙니다.")
    if int(validation["context_length"]) != 756 or float(validation["alpha"]) != 0.5:
        raise RuntimeError("H60 고정 평가의 context 또는 alpha가 고정값과 다릅니다.")
    if bool(validation["alpha_reselected_for_h60"]):
        raise RuntimeError("H60 결과로 alpha를 다시 선택할 수 없습니다.")
    if bool(validation["cross_learning"]):
        raise RuntimeError("H60 고정 평가에서 cross learning을 사용할 수 없습니다.")
...
def requested_origins() -> list[str]:
    return build_monthly_origins(2018, 2021)[:-2]
...
def validate_forecast(forecast: pd.DataFrame, validation: dict[str, object]) -> None:
    ...
    if missing or forecast[list(required)].isna().any().any():
        raise RuntimeError(f"H60 예측에 필수 열이 없거나 결측입니다: {sorted(missing)}")
    if len(forecast) != int(validation["expected_rows"]):
        raise RuntimeError(f"H60 평가 행 수가 예상과 다릅니다: {len(forecast)}")
    if forecast["requested_origin"].nunique() != int(validation["expected_origins"]):
        raise RuntimeError("H60 평가 기준일 수가 예상과 다릅니다.")
    counts = forecast.groupby("requested_origin")["forecast_step"].agg(["size", "nunique"])
    if ((counts["size"] != 60) | (counts["nunique"] != 60)).any():
        raise RuntimeError("H60 기준일별 행 또는 step이 60개가 아닙니다.")
    if forecast.duplicated(["requested_origin", "forecast_step", "target_date"]).any():
        raise RuntimeError("H60 평가 결과에 중복 행이 있습니다.")
    ...
    if dates.min() < pd.Timestamp(validation["target_start"]) or dates.max() > pd.Timestamp(validation["target_end"]):
        raise RuntimeError("H60 평가 결과에 Validation 밖 목표일이 있습니다.")
    if set(forecast["context_length"].astype(int).unique()) != {756}:
        raise RuntimeError("H60 평가 context가 756이 아닙니다.")


def evaluate_forecast(
    forecast: pd.DataFrame, criteria: dict[str, object]
    ...
    checks = {
        ...
        "minimum_origin_mae_wins_vs_random_walk": mae_wins >= int(criteria["minimum_origin_mae_wins_vs_random_walk"]),
        "minimum_origin_rmse_wins_vs_random_walk": rmse_wins >= int(criteria["minimum_origin_rmse_wins_vs_random_walk"]),
    }
    summary = pd.DataFrame([{"split": "validation_2018_2021", "horizon": 60, "alpha": 0.5, "origins": forecast["requested_origin"].nunique(), **metrics}])
    decision = {
        "evaluation": "h60_locked_alpha0.5_validation_2018_2021",
        "alpha": 0.5,
        "alpha_was_reselected_for_h60": False,
        ...
        "passed_h60_service_candidate_criteria": all(checks.values()),
        "next_action": "retain_h60_ensemble_candidate" if all(checks.values()) else "drop_h60_service_candidate_do_not_retune_alpha",
    }
...
def main() -> None:
    ...
    if existing:
        raise FileExistsError(f"기존 H60 고정 평가를 덮어쓰지 않습니다: {existing}")
    ...
    summary, by_origin, by_lead, decision = evaluate_forecast(forecast, config["success_criteria"])
